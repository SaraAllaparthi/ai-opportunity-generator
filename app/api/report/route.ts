...
// New implementations should use /api/research instead
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { name, website, industry } = InputSchema.parse(body)
    await track('brief_started', { name, website, industry })

    console.log('[API] Starting research pipeline for:', { name, website, industry })
    ...
    if (!isDevelopment) {
      ...
      } else if (message.includes('Insufficient public data')) {
        userMessage = 'Unable to find enough public information about this company. Please verify the website URL is correct.'
